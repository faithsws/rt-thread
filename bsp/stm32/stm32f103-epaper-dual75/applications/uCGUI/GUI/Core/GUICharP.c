...
void GUIPROP_DispChar(U16P c) {
  ...
  if (pProp) {
    ...
    const GUI_CHARINFO GUI_UNI_PTR * pCharInfo = NULL;
#if 1
    if(pProp->GetCharInfo)
    {
        pCharInfo = pProp->GetCharInfo(pProp,c);
    }
    else
#endif
    {
...
int GUIPROP_GetCharDistX(U16P c) {
  const GUI_FONT_PROP GUI_UNI_PTR * pProp = GUIPROP_FindChar(GUI_Context.pAFont->p.pProp, c);
#if 1
  if(pProp && pProp->GetCharInfo)
  {
    const GUI_CHARINFO GUI_UNI_PTR * charInfo = pProp->GetCharInfo(pProp,c);
    if(charInfo)
        return charInfo->XSize * GUI_Context.pAFont->XMag;
    else
        return 0;
  }
#endif
