...
void OledFillPoint(int x, int y,uint8_t color)
{
	extern void set_pixel_wb(int x, int y, uint8_t color);
	set_pixel_wb(x,y,color);
}
void gui_draw_qr_at(uint8_t *data, uint8_t mul, uint16_t offset_x, uint16_t offset_y)
{
    uint8_t i, j, x, y;
    EncodeData((char *)data);
    for (i = 0; i < m_nSymbleSize; i++)
    {
        for (j = 0; j < m_nSymbleSize; j++)
        {
            for (x = mul*i; x < mul*i+mul; x++)
            {
                for (y = mul*j; y < mul*j+mul; y++)
                {
                    if (m_byModuleData[i][j])
                    {
                        OledFillPoint(x+offset_x+2, y+offset_y+2, 0);
                    }
                    else
                    {
                        OledFillPoint(x+offset_x+2, y+offset_y+2, 1);
                    }
                }
            }
        }       
    ...
    for (i = offset_x; i < offset_x+m_nSymbleSize*mul+4; i++)
    {
        for (j = offset_y; j < offset_y+2; j++)
        {
            OledFillPoint(i, j, 1);
        } 
    }
    for (i = offset_x; i < offset_x+m_nSymbleSize*mul+4; i++)
    {
        for (j = offset_y+m_nSymbleSize*mul+2; j < offset_y+m_nSymbleSize*mul+4; j++)
        {
            OledFillPoint(i, j, 1);
        } 
    }
    for (i = offset_x; i < offset_x+2; i++)
    {
        for (j = offset_y+2; j < offset_y+m_nSymbleSize*mul+4; j++)
        {
            OledFillPoint(i, j, 1);
        } 
    }
    for (i = offset_x+m_nSymbleSize*mul+2; i < offset_x+m_nSymbleSize*mul+4; i++)
    {
        for (j = offset_y+2; j < offset_y+m_nSymbleSize*mul+4; j++)
        {
            OledFillPoint(i, j, 1);
...
//    extern void epaper_draw_buffer_on_last();
//	epaper_draw_buffer_on_last();
}
